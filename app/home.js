...
export default function home() {
  ...
  function convertTo24Hour(time) {
    ...
    return `${hours}:${minutes}:${seconds || "00"}`; // Return time in 24-hour format with seconds
  }
    ...
    async function fetchData() {
      ...
      let response = await fetch(
        process.env.EXPO_PUBLIC_URL +
          "/Trivio-Back-End/LoadHomeData?id=" +
          user.id
      );
      ...
      if (response.ok) {
        ...
        if (json.success) {
          ...
          let chatHistoryArray = chatArray
            ...
            .sort((a, b) => {
              // Debugging: Log date and time
              //console.log(`dateA: ${a.date}, timeA: ${a.time}`);
              //console.log(`dateB: ${b.date}, timeB: ${b.time}`);

              // Convert date format from YYYY-MMM-DD to YYYY-MM-DD
              const formattedDateA = convertDateFormat(a.date);
              const formattedDateB = convertDateFormat(b.date);

              // Convert time to 24-hour format
              const timeA = convertTo24Hour(a.fullTime);
              const timeB = convertTo24Hour(b.fullTime);

              // Combine date and time for sorting
              const dateTimeA = new Date(`${formattedDateA}T${timeA}`);
              const dateTimeB = new Date(`${formattedDateB}T${timeB}`);

              // Debugging: Log created date objects
              //console.log(`dateTimeA: ${dateTimeA}, dateTimeB: ${dateTimeB}`);
          ...
          //console.log("Sorted Chat History Array:", chatHistoryArray);
        }
      }
    }
